            ...
            <tbody class="divide-y divide-slate-100">
                @forelse ($rows as $row)
                    <tr wire:key="diff-{{ $row->product->id }}">
                        ...
                        <td class="px-4 py-3 text-right font-medium {{ $row->difference != 0 ? 'text-amber-600' : 'text-slate-400' }}">
                            {{ number_format($row->difference, 2) }}
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="6" class="px-4 py-8 text-center text-slate-400">Sin despachos en el periodo seleccionado.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
