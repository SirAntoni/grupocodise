<x-page :title="$invoice ? 'Factura — borrador #'.$invoice->id : 'Nueva factura'">
    @unless ($invoice)
        <div class="bg-white shadow-sm sm:rounded-lg p-6 space-y-4 max-w-xl">
        ...
        <div class="bg-white shadow-sm sm:rounded-lg p-6 space-y-4">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    ...
                    <div class="text-sm text-gray-500 font-mono">{{ $invoice->client->ruc }}</div>
                </div>
            ...
            <div>
                <h3 class="font-semibold text-gray-800 mb-2">Guías consolidadas</h3>
                <div class="flex flex-wrap gap-2">
                    @foreach ($invoice->dispatchGuides as $guide)
                        <span class="inline-flex items-center gap-2 bg-indigo-50 text-indigo-800 rounded-full px-3 py-1 text-sm font-mono"
                              wire:key="chip-{{ $guide->id }}">
                            {{ $guide->full_number }}
                            <button wire:click="removeGuide({{ $guide->id }})" class="text-indigo-400 hover:text-red-600" title="Quitar">✕</button>
                        </span>
                    @endforeach
                </div>
        ...
        <div class="bg-white shadow-sm sm:rounded-lg overflow-x-auto">
            ...
            <div class="p-4 border-t border-gray-100 flex justify-end">
                <div class="w-72 space-y-1 text-sm">
                    <div class="flex justify-between"><span class="text-gray-500">Op. gravadas</span><span>S/ {{ number_format((float) $invoice->taxable_amount, 2) }}</span></div>
                    <div class="flex justify-between"><span class="text-gray-500">IGV (18%)</span><span>S/ {{ number_format((float) $invoice->igv, 2) }}</span></div>
                    <div class="flex justify-between font-semibold text-base border-t border-gray-200 pt-1"><span>Total</span><span>S/ {{ number_format((float) $invoice->total, 2) }}</span></div>
                </div>
            </div>
        </div>
