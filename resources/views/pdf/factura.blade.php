...
<body>
        ...
        <table width="100%">
            <tr>
                ...
                <td width="220" style="vertical-align: top;">
                    <span class="label">Fecha de emisión:</span> {{ $invoice->issue_date->format('d/m/Y') }}<br>
    ...
    <div class="legend">{{ \App\Support\NumeroALetras::legend((float) $invoice->total) }}</div>

    <table class="totals">
        <tr><td class="label">Op. gravadas:</td><td class="right">S/ {{ number_format((float) $invoice->taxable_amount, 2) }}</td></tr>
        <tr><td class="label">IGV (18%):</td><td class="right">S/ {{ number_format((float) $invoice->igv, 2) }}</td></tr>
        <tr class="total-row"><td>IMPORTE TOTAL:</td><td class="right">S/ {{ number_format((float) $invoice->total, 2) }}</td></tr>
    </table>

    @if ($invoice->dispatchGuides->isNotEmpty())
