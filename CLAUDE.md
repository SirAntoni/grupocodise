...
- Tests: `php artisan test` (Pest) sobre **MySQL**, base `ferreteria_testing` (créala una vez; `phpunit.xml` la apunta). **No usar SQLite**: no valida el largo de los varchar, ni los CHECK, ni el rango de los enteros, y dejó pasar a producción errores 500 que solo aparecían con MySQL (incidente del 2026-08-04: código temporal de 40 caracteres en `varchar(20)`, y movimiento de stock con cantidad 0 contra el CHECK de `stock_movements`). `tests/Feature/SchemaLimitsTest.php` cubre esas regresiones.
